			...
			MyAPIGateway.Parallel.Start(delegate
			{
				if (IsServer)
				{
					...
					foreach (IMyCubeGrid CoreGrid in GridList)
					{
						...
						if (Beacons.Count() < 1)
						{
							Globals.PenaliseInvalidGrid(CoreGrid);
						...
						if (MyShipCore == null)
						{
							MyLog.Default.WriteLine("Blues_Ship_Matrix: SHOULD NOT HAPPEN: ShipCore = null");
							continue;
						}

						//If no currently set ship class, pick one
						//This doesn't seem to actually happen?
						if (MyShipCore.CoreGridClass == null)
						{
							MyAPIGateway.Utilities.ShowMessage("Blues_Ship_Matrix: ", "picking ship class");
							MyShipCore.CoreGridClass = Globals.GetClass(CoreGrid);
						}
						...
						bool Penalize = false;
						//Currently, nothing is done with the warning string?
						string Warning = "\n<<< Class Limits Info: >>>\nClass Name:" + CoreGridClass.Name;
						string PerStatWarning = "";
						string PerBlockWarning = "";

						//This updates the warning text shown in the beacon GUI
						if (ticks % 3 == 0)
						{
							try
							{
							...
							{
								MyLog.Default.WriteLine($"BlueShipMatrix: Error @BlockLimits - {e.Message}");
							}
						}
						...
						if (ticks % 6 == 0)
						{
							try
							{
								LimitCheckResults GridStatLimit = Globals.CheckGridStatLimits(CoreBeacon, CoreGridClass);

								if (GridStatLimit.Penalty)
								{
									Penalize = GridStatLimit.Penalty;
								}
							...
							{
								MyLog.Default.WriteLine($"BlueShipMatrix: Error @GridStats - {e.Message}");
							}
						}
						if (ticks % 12 == 0)
						{
							...
							try
							{
								LimitCheckResults ClassLimit = Globals.CheckClassLimits(CoreBeacon, CoreGridClass);

								if (ClassLimit.Penalty)
								{
									Penalize = ClassLimit.Penalty;
								}
						...
						*/
						...
						try
						{
							if (Penalize)
							{
								Globals.PenaliseInvalidGrid(CoreBeacon);
							}
						}
