...
namespace Blues_Ship_Matrix
{
	...
	public static class Globals
	{
		...
		public static LimitCheckResults CheckGridStatLimits(IMyBeacon CoreBeacon, MyGridLimit CoreGridClass)
		{
			bool Penalize = false;
			...
			return new LimitCheckResults { Penalty = Penalize, Warning = warningMessage };

		}
		public static LimitCheckResults CheckClassLimits(IMyBeacon CoreBeacon, MyGridLimit CoreGridClass)
		{
			bool Penalize = false;
			string warningMessage = "";
			IMyCubeGrid CoreGrid = CoreBeacon.CubeGrid;
		...
		//All this does is create a warning string which is shown in the beacon GUI
		public static string CheckAndPenalizeBlockLimits(IMyBeacon CoreBeacon, MyGridLimit CoreGridClass)
		{
			...
			foreach (MyBlockLimit BlockLimit in CoreGridClass.BlockLimits)
			{
				...
				foreach (IMyFunctionalBlock Block in BlocksOnGrid)
				{
					...
					foreach (MyBlockId BlockId in BlockLimit.BlockIds)
					{
		...
		public static void PenaliseInvalidGrid(IMyCubeBlock Block)
		{
			PenaliseInvalidGrid(Block.CubeGrid);
			// IEnumerable<IMyFunctionalBlock> BlocksOnGrid = CoreBeacon.CubeGrid.GetFatBlocks<IMyFunctionalBlock>();
